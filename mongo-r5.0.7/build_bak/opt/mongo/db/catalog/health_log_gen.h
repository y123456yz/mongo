...
static constexpr uint32_t kNumSeverityEnum = 3;
...
static constexpr uint32_t kNumScopeEnum = 6;
...
public:
    ...
    HealthLogEntry();
    HealthLogEntry(mongo::Date_t timestamp, mongo::SeverityEnum severity, std::string msg, mongo::ScopeEnum scope, std::string operation);

    static HealthLogEntry parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject);
    void serialize(BSONObjBuilder* builder) const;
    BSONObj toBSON() const;

    const boost::optional<mongo::NamespaceString>& getNss() const& { return _nss; }
    void getNss() && = delete;
    void setNss(boost::optional<mongo::NamespaceString> value) & {  _nss = std::move(value);  }
    ...
    const boost::optional<mongo::BSONObj>& getData() const& { return _data; }
    void getData() && = delete;
    void setData(boost::optional<mongo::BSONObj> value) & {  _data = std::move(value);  }
...
private:
    ...
    boost::optional<mongo::BSONObj> _data;

...
HttpClientReply::HttpClientReply() : _code(mongo::idl::preparsedValue<decltype(_code)>()), _header(mongo::idl::preparsedValue<decltype(_header)>()), _body(mongo::idl::preparsedValue<decltype(_body)>()), _hasCode(false), _hasHeader(false), _hasBody(false) {
...
HttpClientReply HttpClientReply::parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    auto object = mongo::idl::preparsedValue<HttpClientReply>();
...
HttpClientRequest::HttpClientRequest() : _uri(mongo::idl::preparsedValue<decltype(_uri)>()), _hasUri(false), _hasDbName(false) {
    // Used for initialization only
}
...
HttpClientRequest HttpClientRequest::parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    auto object = mongo::idl::preparsedValue<HttpClientRequest>();
    object.parseProtected(ctxt, bsonObject);
    return object;
}

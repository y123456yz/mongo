...
public:
    ...
    explicit MapReduceCommandRequest(const NamespaceString nss);
    MapReduceCommandRequest(const NamespaceString nss, mongo::MapReduceJavascriptCode map, mongo::MapReduceJavascriptCode reduce, mongo::MapReduceOutOptions outOptions);

    static MapReduceCommandRequest parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject);
    static MapReduceCommandRequest parse(const IDLParserErrorContext& ctxt, const OpMsgRequest& request);

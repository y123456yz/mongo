...
public:
    ...
    /**
     * A query predicate that determines which ops are logged/profiled.  This is an alternative to slowms and sampleRate. The special value 'unset' removes the filter.
     */
    const boost::optional<mongo::ObjectOrUnset>& getFilter() const& { return _filter; }
    void getFilter() && = delete;
    void setFilter(boost::optional<mongo::ObjectOrUnset> value) & {  _filter = std::move(value);  }
...
private:
    ...
    boost::optional<mongo::ObjectOrUnset> _filter;
    std::string _dbName;
    bool _hasDbName : 1;
};

...
TxnResponseMetadata::TxnResponseMetadata() : _readOnly(mongo::idl::preparsedValue<decltype(_readOnly)>()), _hasReadOnly(false) {
...
TxnResponseMetadata TxnResponseMetadata::parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    auto object = mongo::idl::preparsedValue<TxnResponseMetadata>();
...
TxnRecoveryToken TxnRecoveryToken::parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    auto object = mongo::idl::preparsedValue<TxnRecoveryToken>();
    object.parseProtected(ctxt, bsonObject);
    return object;
}
...
CommitTransactionOplogObject CommitTransactionOplogObject::parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    auto object = mongo::idl::preparsedValue<CommitTransactionOplogObject>();
    object.parseProtected(ctxt, bsonObject);
    return object;
}
...
AbortTransactionOplogObject AbortTransactionOplogObject::parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    auto object = mongo::idl::preparsedValue<AbortTransactionOplogObject>();
    object.parseProtected(ctxt, bsonObject);
    return object;
}
...
mongo::AuthorizationContract CommitTransaction::kAuthorizationContract = AuthorizationContract(std::initializer_list<AccessCheckEnum>{}, std::initializer_list<Privilege>{});
...
CommitTransaction CommitTransaction::parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    auto object = mongo::idl::preparsedValue<CommitTransaction>();
    object.parseProtected(ctxt, bsonObject);
    return object;
}
...
constexpr StringData AbortTransaction::kRecoveryTokenFieldName;
constexpr StringData AbortTransaction::kCommandName;

mongo::AuthorizationContract AbortTransaction::kAuthorizationContract = AuthorizationContract(std::initializer_list<AccessCheckEnum>{}, std::initializer_list<Privilege>{});

const std::vector<StringData> AbortTransaction::_knownBSONFields {
    AbortTransaction::kRecoveryTokenFieldName,
...
void AbortTransaction::parseProtected(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    ...
    for (const auto& element :bsonObject) {
        ...
        if (fieldName == kRecoveryTokenFieldName) {
            if (MONGO_likely(ctxt.checkAndAssertType(element, Object))) {
                if (MONGO_unlikely(usedFields[kRecoveryTokenBit])) {
                    ctxt.throwDuplicateField(element);
                }

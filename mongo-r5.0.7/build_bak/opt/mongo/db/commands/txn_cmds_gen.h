...
public:
    ...
    /**
     * A mongos that doesn't know about this transaction can attempt to make progress on abort by processing using the info in the recoveryToken.
     */
    const boost::optional<mongo::TxnRecoveryToken>& getRecoveryToken() const& { return _recoveryToken; }
    void getRecoveryToken() && = delete;
    void setRecoveryToken(boost::optional<mongo::TxnRecoveryToken> value) & {  _recoveryToken = std::move(value);  }
...
class CommitTransactionCmdVersion1Gen : public TypedCommand<Derived> {
...
public:
    ...
    virtual ~CommitTransactionCmdVersion1Gen() = default;
...
template <typename Derived>
class AbortTransactionCmdVersion1Gen : public TypedCommand<Derived> {
    using _TypedCommandInvocationBase = typename TypedCommand<Derived>::InvocationBase;
...
public:
    ...
    virtual ~AbortTransactionCmdVersion1Gen() = default;

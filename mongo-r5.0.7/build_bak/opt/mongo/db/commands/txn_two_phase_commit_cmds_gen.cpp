...
CommitParticipant::CommitParticipant() : _shardId(mongo::idl::preparsedValue<decltype(_shardId)>()), _hasShardId(false) {
...
CommitParticipant CommitParticipant::parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    auto object = mongo::idl::preparsedValue<CommitParticipant>();
...
PrepareTransaction PrepareTransaction::parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    auto object = mongo::idl::preparsedValue<PrepareTransaction>();
    object.parseProtected(ctxt, bsonObject);
    return object;
}

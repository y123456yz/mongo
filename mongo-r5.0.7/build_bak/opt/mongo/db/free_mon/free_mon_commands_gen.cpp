...
SetFreeMonitoring::SetFreeMonitoring() : _action(mongo::idl::preparsedValue<decltype(_action)>()), _hasAction(false), _hasDbName(false) {
...
SetFreeMonitoring SetFreeMonitoring::parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    auto object = mongo::idl::preparsedValue<SetFreeMonitoring>();
...
void SetFreeMonitoring::serialize(const BSONObj& commandPassthroughFields, BSONObjBuilder* builder) const {
    ...
    {
        builder->append(kActionFieldName, ::mongo::SetFreeMonAction_serializer(_action));
    }
...
GetFreeMonitoringStatus GetFreeMonitoringStatus::parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    auto object = mongo::idl::preparsedValue<GetFreeMonitoringStatus>();
    object.parseProtected(ctxt, bsonObject);
    return object;
}

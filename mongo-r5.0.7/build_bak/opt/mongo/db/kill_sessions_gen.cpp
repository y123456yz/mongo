...
KillSessionsCmdFromClient KillSessionsCmdFromClient::parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    auto object = mongo::idl::preparsedValue<KillSessionsCmdFromClient>();
...
KillAllSessionsUser::KillAllSessionsUser() : _user(mongo::idl::preparsedValue<decltype(_user)>()), _db(mongo::idl::preparsedValue<decltype(_db)>()), _hasUser(false), _hasDb(false) {
    // Used for initialization only
}
...
KillAllSessionsUser KillAllSessionsUser::parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    auto object = mongo::idl::preparsedValue<KillAllSessionsUser>();
    object.parseProtected(ctxt, bsonObject);
    return object;
}
...
KillAllSessionsRole::KillAllSessionsRole() : _role(mongo::idl::preparsedValue<decltype(_role)>()), _db(mongo::idl::preparsedValue<decltype(_db)>()), _hasRole(false), _hasDb(false) {
    // Used for initialization only
}
...
KillAllSessionsRole KillAllSessionsRole::parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    auto object = mongo::idl::preparsedValue<KillAllSessionsRole>();
    object.parseProtected(ctxt, bsonObject);
    return object;
}
...
void KillAllSessionsCmd::parseProtected(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    ...
    for (const auto& element :bsonObject) {
        ...
        if (fieldName == kKillAllSessionsFieldName) {
            if (MONGO_likely(ctxt.checkAndAssertType(element, Array))) {
                if (MONGO_unlikely(usedFields[kKillAllSessionsBit])) {
                    ctxt.throwDuplicateField(element);
                }

                usedFields.set(kKillAllSessionsBit);
    ...
    if (MONGO_unlikely(!usedFields.all())) {
        if (!usedFields[kKillAllSessionsBit]) {
...
KillAllSessionsByPattern KillAllSessionsByPattern::parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    auto object = mongo::idl::preparsedValue<KillAllSessionsByPattern>();
    object.parseProtected(ctxt, bsonObject);
    return object;
}
...
KillAllSessionsByPatternCmd KillAllSessionsByPatternCmd::parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    auto object = mongo::idl::preparsedValue<KillAllSessionsByPatternCmd>();
    object.parseProtected(ctxt, bsonObject);
    return object;
}
void KillAllSessionsByPatternCmd::parseProtected(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    std::bitset<1> usedFields;
    const size_t kKillAllSessionsByPatternBit = 0;
    std::set<StringData> usedFieldSet;
    ...
    for (const auto& element :bsonObject) {
        ...
        if (fieldName == kKillAllSessionsByPatternFieldName) {
            if (MONGO_likely(ctxt.checkAndAssertType(element, Array))) {
                if (MONGO_unlikely(usedFields[kKillAllSessionsByPatternBit])) {
                    ctxt.throwDuplicateField(element);
                }

                usedFields.set(kKillAllSessionsByPatternBit);
    ...
    if (MONGO_unlikely(!usedFields.all())) {
        if (!usedFields[kKillAllSessionsByPatternBit]) {
...
KillSessionsCmdToServer KillSessionsCmdToServer::parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    auto object = mongo::idl::preparsedValue<KillSessionsCmdToServer>();
    object.parseProtected(ctxt, bsonObject);
    return object;
}

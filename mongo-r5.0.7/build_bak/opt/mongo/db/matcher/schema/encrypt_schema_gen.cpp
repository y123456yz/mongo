...
EncryptionInfo EncryptionInfo::parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    auto object = mongo::idl::preparsedValue<EncryptionInfo>();
...
void EncryptionInfo::serialize(BSONObjBuilder* builder) const {
    ...
    if (_algorithm.is_initialized()) {
        builder->append(kAlgorithmFieldName, ::mongo::FleAlgorithm_serializer(_algorithm.get()));
    }
...
EncryptionMetadata EncryptionMetadata::parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    auto object = mongo::idl::preparsedValue<EncryptionMetadata>();
    object.parseProtected(ctxt, bsonObject);
    return object;
}
...
EncryptionPlaceholder::EncryptionPlaceholder() : _algorithm(mongo::idl::preparsedValue<decltype(_algorithm)>()), _value(mongo::idl::preparsedValue<decltype(_value)>()), _hasAlgorithm(false), _hasValue(false) {
    // Used for initialization only
}
EncryptionPlaceholder::EncryptionPlaceholder(mongo::FleAlgorithmInt algorithm, mongo::IDLAnyType value) : _algorithm(std::move(algorithm)), _value(std::move(value)), _hasAlgorithm(true), _hasValue(true) {
    // Used for initialization only
}


EncryptionPlaceholder EncryptionPlaceholder::parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    auto object = mongo::idl::preparsedValue<EncryptionPlaceholder>();
    object.parseProtected(ctxt, bsonObject);
    return object;
}
void EncryptionPlaceholder::parseProtected(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    ...
    for (const auto& element :bsonObject) {
        ...
        if (fieldName == kAlgorithmFieldName) {
            ...
            _hasValue = true;
            _value = IDLAnyType::parseFromBSON(element);
        }
        else {
            ctxt.throwUnknownField(fieldName);
        }
...
void EncryptionPlaceholder::serialize(BSONObjBuilder* builder) const {
    ...
    {
        builder->append(kAlgorithmFieldName, ::mongo::FleAlgorithmInt_serializer(_algorithm));
    }

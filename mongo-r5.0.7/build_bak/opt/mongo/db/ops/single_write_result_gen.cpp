...
SingleWriteResult::SingleWriteResult() : _n(mongo::idl::preparsedValue<decltype(_n)>()), _nModified(mongo::idl::preparsedValue<decltype(_nModified)>()), _upsertedId(mongo::idl::preparsedValue<decltype(_upsertedId)>()), _hasN(false), _hasNModified(false), _hasUpsertedId(false) {
...
SingleWriteResult SingleWriteResult::parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    auto object = mongo::idl::preparsedValue<SingleWriteResult>();

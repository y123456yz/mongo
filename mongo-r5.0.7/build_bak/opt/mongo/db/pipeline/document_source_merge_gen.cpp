...
NamespaceSpec NamespaceSpec::parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    auto object = mongo::idl::preparsedValue<NamespaceSpec>();
...
DocumentSourceMergeSpec::DocumentSourceMergeSpec() : _targetNss(mongo::idl::preparsedValue<decltype(_targetNss)>()), _hasTargetNss(false) {
    // Used for initialization only
}
...
DocumentSourceMergeSpec DocumentSourceMergeSpec::parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    auto object = mongo::idl::preparsedValue<DocumentSourceMergeSpec>();
    object.parseProtected(ctxt, bsonObject);
    return object;
}
...
void DocumentSourceMergeSpec::serialize(BSONObjBuilder* builder) const {
    ...
    if (_whenNotMatched.is_initialized()) {
        builder->append(kWhenNotMatchedFieldName, ::mongo::MergeWhenNotMatchedMode_serializer(_whenNotMatched.get()));
    }

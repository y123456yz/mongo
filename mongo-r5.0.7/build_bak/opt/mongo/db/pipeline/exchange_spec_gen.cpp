...
ExchangeSpec::ExchangeSpec() : _policy(mongo::idl::preparsedValue<decltype(_policy)>()), _consumers(mongo::idl::preparsedValue<decltype(_consumers)>()), _hasPolicy(false), _hasConsumers(false) {
...
ExchangeSpec ExchangeSpec::parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    auto object = mongo::idl::preparsedValue<ExchangeSpec>();
...
void ExchangeSpec::serialize(BSONObjBuilder* builder) const {
    ...
    {
        builder->append(kPolicyFieldName, ::mongo::ExchangePolicy_serializer(_policy));
    }

...
public:
    ...
    explicit CountCommandRequest(const NamespaceStringOrUUID nssOrUUID);

    static CountCommandRequest parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject);
    static CountCommandRequest parse(const IDLParserErrorContext& ctxt, const OpMsgRequest& request);

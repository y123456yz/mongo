...
static constexpr uint32_t kNumOplogEntriesEnum = 4;
...
public:
    static constexpr auto kBatchWriteConcernFieldName = "batchWriteConcern"_sd;
    static constexpr auto kCollFieldName = "dbCheck"_sd;
    static constexpr auto kMaxBatchTimeMillisFieldName = "maxBatchTimeMillis"_sd;
    static constexpr auto kMaxBytesPerBatchFieldName = "maxBytesPerBatch"_sd;
    ...
    static constexpr auto kMaxDocsPerBatchFieldName = "maxDocsPerBatch"_sd;
    ...
    static constexpr auto kSnapshotReadFieldName = "snapshotRead"_sd;

    DbCheckSingleInvocation();
    DbCheckSingleInvocation(std::string coll);
    ...
    std::int64_t getMaxDocsPerBatch() const { return _maxDocsPerBatch; }
    void setMaxDocsPerBatch(std::int64_t value) & { validateMaxDocsPerBatch(value); _maxDocsPerBatch = std::move(value);  }

    std::int64_t getMaxBytesPerBatch() const { return _maxBytesPerBatch; }
    void setMaxBytesPerBatch(std::int64_t value) & { validateMaxBytesPerBatch(value); _maxBytesPerBatch = std::move(value);  }

    std::int64_t getMaxBatchTimeMillis() const { return _maxBatchTimeMillis; }
    void setMaxBatchTimeMillis(std::int64_t value) & { validateMaxBatchTimeMillis(value); _maxBatchTimeMillis = std::move(value);  }

    bool getSnapshotRead() const { return _snapshotRead; }
    void setSnapshotRead(bool value) & {  _snapshotRead = std::move(value);  }

    /**
     * Wait for this writeConcern at the end of every batch. Default is w:1 with no timeout.
     */
    const mongo::WriteConcernOptions& getBatchWriteConcern() const { return _batchWriteConcern; }
    void setBatchWriteConcern(mongo::WriteConcernOptions value) & {  _batchWriteConcern = std::move(value);  }

protected:
    void parseProtected(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject);

private:
    void validateMaxDocsPerBatch(const std::int64_t value);
    void validateMaxDocsPerBatch(IDLParserErrorContext& ctxt, const std::int64_t value);

    void validateMaxBytesPerBatch(const std::int64_t value);
    void validateMaxBytesPerBatch(IDLParserErrorContext& ctxt, const std::int64_t value);

    void validateMaxBatchTimeMillis(const std::int64_t value);
    void validateMaxBatchTimeMillis(IDLParserErrorContext& ctxt, const std::int64_t value);
...
private:
    ...
    std::int64_t _maxDocsPerBatch{5000};
    std::int64_t _maxBytesPerBatch{20 * 1024 * 1024};
    std::int64_t _maxBatchTimeMillis{1000};
    bool _snapshotRead{true};
    mongo::WriteConcernOptions _batchWriteConcern{WriteConcernOptions()};
    bool _hasColl : 1;
};
...
public:
    ...
    DbCheckAllInvocation(std::int64_t tag);
...
public:
    ...
    static constexpr auto kReadTimestampFieldName = "readTimestamp"_sd;
    static constexpr auto kTypeFieldName = "type"_sd;

    DbCheckOplogBatch();
    DbCheckOplogBatch(mongo::NamespaceString nss, mongo::OplogEntriesEnum type, std::string md5, mongo::BSONKey minKey, mongo::BSONKey maxKey);
    ...
    mongo::OplogEntriesEnum getType() const { return _type; }
    void setType(mongo::OplogEntriesEnum value) & {  _type = std::move(value); _hasType = true; }

    const StringData getMd5() const& { return _md5; }
    void getMd5() && = delete;
    void setMd5(StringData value) & { auto _tmpValue = value.toString();  _md5 = std::move(_tmpValue); _hasMd5 = true; }
    ...
    const boost::optional<mongo::Timestamp>& getReadTimestamp() const& { return _readTimestamp; }
    void getReadTimestamp() && = delete;
    void setReadTimestamp(boost::optional<mongo::Timestamp> value) & {  _readTimestamp = std::move(value);  }
...
public:
    ...
    DbCheckOplogCollection(mongo::NamespaceString nss, mongo::OplogEntriesEnum type, mongo::UUID uuid, std::vector<mongo::BSONObj> indexes, mongo::BSONObj options);
...
/**
 * Oplog entry for dbCheck start and stop
 */
class DbCheckOplogStartStop {
public:
    static constexpr auto kNssFieldName = "dbCheck"_sd;
    static constexpr auto kTypeFieldName = "type"_sd;

    DbCheckOplogStartStop();
    DbCheckOplogStartStop(mongo::NamespaceString nss, mongo::OplogEntriesEnum type);

    static DbCheckOplogStartStop parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject);
    void serialize(BSONObjBuilder* builder) const;
    BSONObj toBSON() const;
...
constexpr auto kDbCheckCollectionTryLockTimeoutMillisDefault = 10;

extern AtomicWord<int> gDbCheckCollectionTryLockTimeoutMillis;
constexpr auto kDbCheckCollectionTryLockMaxAttemptsDefault = 5;

extern AtomicWord<int> gDbCheckCollectionTryLockMaxAttempts;
constexpr auto kDbCheckCollectionTryLockMinBackoffMillisDefault = 10;

extern AtomicWord<int> gDbCheckCollectionTryLockMinBackoffMillis;
constexpr auto kDbCheckCollectionTryLockMaxBackoffMillisDefault = 60000;

extern AtomicWord<int> gDbCheckCollectionTryLockMaxBackoffMillis;
constexpr auto kDbCheckHealthLogEveryNBatchesDefault = 25;

extern AtomicWord<int> gDbCheckHealthLogEveryNBatches;
}  // namespace mongo

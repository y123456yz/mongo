...
RollbackID::RollbackID() : __id(mongo::idl::preparsedValue<decltype(__id)>()), _rollbackId(mongo::idl::preparsedValue<decltype(_rollbackId)>()), _has_id(false), _hasRollbackId(false) {
...
RollbackID RollbackID::parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    auto object = mongo::idl::preparsedValue<RollbackID>();
...
RollbackProgress::RollbackProgress() : __id(mongo::idl::preparsedValue<decltype(__id)>()), _applyUntil(mongo::idl::preparsedValue<decltype(_applyUntil)>()), _has_id(false), _hasApplyUntil(false) {
    // Used for initialization only
}
...
RollbackProgress RollbackProgress::parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    auto object = mongo::idl::preparsedValue<RollbackProgress>();
    object.parseProtected(ctxt, bsonObject);
    return object;
}

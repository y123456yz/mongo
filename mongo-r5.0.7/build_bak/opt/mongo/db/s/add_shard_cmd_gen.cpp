...
ShardIdentity::ShardIdentity() : _shardName(mongo::idl::preparsedValue<decltype(_shardName)>()), _clusterId(mongo::idl::preparsedValue<decltype(_clusterId)>()), _configsvrConnectionString(mongo::idl::preparsedValue<decltype(_configsvrConnectionString)>()), _hasShardName(false), _hasClusterId(false), _hasConfigsvrConnectionString(false) {
...
ShardIdentity ShardIdentity::parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    auto object = mongo::idl::preparsedValue<ShardIdentity>();
...
AddShard::AddShard() : _shardIdentity(mongo::idl::preparsedValue<decltype(_shardIdentity)>()), _hasShardIdentity(false), _hasDbName(false) {
    // Used for initialization only
}
...
AddShard AddShard::parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    auto object = mongo::idl::preparsedValue<AddShard>();
    object.parseProtected(ctxt, bsonObject);
    return object;
}

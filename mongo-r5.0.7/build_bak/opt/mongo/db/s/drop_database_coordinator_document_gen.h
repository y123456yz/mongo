 ...
 */
enum class DropDatabaseCoordinatorPhaseEnum : std::int32_t {
    kUnset ,
    kDrop ,
};
static constexpr uint32_t kNumDropDatabaseCoordinatorPhaseEnum = 2;
...
/**
 * Represents a drop database operation on the coordinator shard.
 */
class DropDatabaseCoordinatorDocument {
public:
    ...
    static DropDatabaseCoordinatorDocument parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject);
    ...
    mongo::DropDatabaseCoordinatorPhaseEnum getPhase() const { return _phase; }
    void setPhase(mongo::DropDatabaseCoordinatorPhaseEnum value) & {  _phase = std::move(value);  }

    /**
     * Collection currently being dropped.
     */
...
private:
    ...
    mongo::DropDatabaseCoordinatorPhaseEnum _phase{mongo::DropDatabaseCoordinatorPhaseEnum::kUnset};

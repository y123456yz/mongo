...
CoordinatorCommitDecision::CoordinatorCommitDecision() : _decision(mongo::idl::preparsedValue<decltype(_decision)>()), _hasDecision(false) {
...
CoordinatorCommitDecision CoordinatorCommitDecision::parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    auto object = mongo::idl::preparsedValue<CoordinatorCommitDecision>();
...
void CoordinatorCommitDecision::serialize(BSONObjBuilder* builder) const {
    ...
    {
        builder->append(kDecisionFieldName, ::mongo::txn::writeCommitDecisionEnumProperty(_decision));
    }
...
TransactionCoordinatorDocument::TransactionCoordinatorDocument() : _id(mongo::idl::preparsedValue<decltype(_id)>()), _hasId(false), _hasParticipants(false) {
    // Used for initialization only
}
...
TransactionCoordinatorDocument TransactionCoordinatorDocument::parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    auto object = mongo::idl::preparsedValue<TransactionCoordinatorDocument>();
    object.parseProtected(ctxt, bsonObject);
    return object;
}

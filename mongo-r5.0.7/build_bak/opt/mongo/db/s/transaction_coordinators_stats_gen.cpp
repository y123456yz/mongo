...
CurrentInSteps CurrentInSteps::parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    auto object = mongo::idl::preparsedValue<CurrentInSteps>();
...
TransactionCoordinatorsStats::TransactionCoordinatorsStats() : _currentInSteps(mongo::idl::preparsedValue<decltype(_currentInSteps)>()), _hasCurrentInSteps(false) {
    // Used for initialization only
}
...
TransactionCoordinatorsStats TransactionCoordinatorsStats::parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    auto object = mongo::idl::preparsedValue<TransactionCoordinatorsStats>();
    object.parseProtected(ctxt, bsonObject);
    return object;
}

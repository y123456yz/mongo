/**
 * WARNING: This is a generated file. Do not modify.
 *
 * Source: buildscripts/idl/idlc.py --include src --base_dir build/opt --target_arch x86_64 --header build/opt/mongo/db/storage/storage_parameters_gen.h --output build/opt/mongo/db/storage/storage_parameters_gen.cpp src/mongo/db/storage/storage_parameters.idl
 */

#include "mongo/platform/basic.h"

#include "mongo/db/storage/storage_parameters_gen.h"

#include <bitset>
#include <set>

#include "mongo/bson/bsonobjbuilder.h"
#include "mongo/db/auth/authorization_contract.h"
#include "mongo/db/commands.h"
#include "mongo/idl/command_generic_argument.h"
#include "mongo/idl/server_parameter.h"
#include "mongo/idl/server_parameter_with_storage.h"

namespace mongo {

::mongo::FeatureFlag feature_flags::gLockFreeReads{true, "4.9"_sd};
::mongo::FeatureFlag feature_flags::gTimeseriesCollection{true, "5.0"_sd};
::mongo::FeatureFlag feature_flags::gTimeseriesUpdatesAndDeletes{true, "5.0"_sd};
bool gTakeUnstableCheckpointOnShutdown{false};
AtomicWord<int32_t> gOperationMemoryPoolBlockInitialSizeKB{1};
AtomicWord<int32_t> gOperationMemoryPoolBlockMaxSizeKB{2048};
MONGO_SERVER_PARAMETER_REGISTER(idl_00f530d5d6eec2fd73d1b0b17c85c58438bf160b)(InitializerContext*) {
    /**
     * Support for lock-free reads.
     */
    [[maybe_unused]] auto* scp_0 = ([]() -> ServerParameter* {
        auto* ret = new FeatureFlagServerParameter("featureFlagLockFreeReads", feature_flags::gLockFreeReads);
        return ret;
    })();

    /**
     * When enabled, support for time-series collections
     */
    [[maybe_unused]] auto* scp_1 = ([]() -> ServerParameter* {
        auto* ret = new FeatureFlagServerParameter("featureFlagTimeseriesCollection", feature_flags::gTimeseriesCollection);
        return ret;
    })();

    /**
     * When enabled, support for updates and deletes on time-series collections
     */
    [[maybe_unused]] auto* scp_2 = ([]() -> ServerParameter* {
        auto* ret = new FeatureFlagServerParameter("featureFlagTimeseriesUpdatesAndDeletes", feature_flags::gTimeseriesUpdatesAndDeletes);
        return ret;
    })();

    /**
     * Whether all queries must use indexes. If true, MongoDB will not execute queries that require a table scan and will return an error.
     */
    [[maybe_unused]] auto* scp_3 = ([]() -> ServerParameter* {
        auto* ret = makeIDLServerParameterWithStorage<ServerParameterType::kStartupAndRuntime>("notablescan", storageGlobalParams.noTableScan);
        return ret;
    })();

    /**
     * Interval in seconds between fsync operations where mongod flushes its working memory to disk. By default, mongod flushes memory to disk every 60 seconds.
     */
    [[maybe_unused]] auto* scp_4 = ([]() -> ServerParameter* {
        auto* ret = makeIDLServerParameterWithStorage<ServerParameterType::kStartupAndRuntime>("syncdelay", storageGlobalParams.syncdelay);
        ret->addBound<idl_server_parameter_detail::LTE>(([]{ constexpr auto value = StorageGlobalParams::kMaxSyncdelaySecs; return value; })());
        ret->addBound<idl_server_parameter_detail::GTE>(0.0);
        return ret;
    })();

    /**
     * Number of milliseconds between journal commits
     */
    [[maybe_unused]] auto* scp_5 = ([]() -> ServerParameter* {
        auto* ret = makeIDLServerParameterWithStorage<ServerParameterType::kStartupAndRuntime>("journalCommitInterval", storageGlobalParams.journalCommitIntervalMs);
        ret->addBound<idl_server_parameter_detail::LTE>(([]{ constexpr auto value = StorageGlobalParams::kMaxJournalCommitIntervalMs; return value; })());
        ret->addBound<idl_server_parameter_detail::GTE>(1);
        return ret;
    })();

    /**
     * Take unstable checkpoint on shutdown
     */
    [[maybe_unused]] auto* scp_6 = ([]() -> ServerParameter* {
        auto* ret = makeIDLServerParameterWithStorage<ServerParameterType::kStartupOnly>("takeUnstableCheckpointOnShutdown", gTakeUnstableCheckpointOnShutdown);
        return ret;
    })();

    /**
     * Initial block size in KB for the per operation temporary object memory pool
     */
    [[maybe_unused]] auto* scp_7 = ([]() -> ServerParameter* {
        auto* ret = makeIDLServerParameterWithStorage<ServerParameterType::kStartupAndRuntime>("operationMemoryPoolBlockInitialSizeKB", gOperationMemoryPoolBlockInitialSizeKB);
        ret->addBound<idl_server_parameter_detail::GTE>(1);
        return ret;
    })();

    /**
     * Maximum block size in KB for the per operation temporary object memory pool
     */
    [[maybe_unused]] auto* scp_8 = ([]() -> ServerParameter* {
        auto* ret = makeIDLServerParameterWithStorage<ServerParameterType::kStartupAndRuntime>("operationMemoryPoolBlockMaxSizeKB", gOperationMemoryPoolBlockMaxSizeKB);
        ret->addBound<idl_server_parameter_detail::GTE>(1);
        return ret;
    })();

}
}  // namespace mongo

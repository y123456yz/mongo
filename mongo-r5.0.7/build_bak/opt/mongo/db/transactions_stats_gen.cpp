...
LastCommittedTransaction::LastCommittedTransaction() : _writeConcern(mongo::idl::preparsedValue<decltype(_writeConcern)>()), _hasWriteConcern(false) {
...
LastCommittedTransaction LastCommittedTransaction::parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    auto object = mongo::idl::preparsedValue<LastCommittedTransaction>();
...
TransactionsStats TransactionsStats::parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    auto object = mongo::idl::preparsedValue<TransactionsStats>();
    object.parseProtected(ctxt, bsonObject);
    return object;
}

...
constexpr StringData kReadWriteConcernProvenanceSource_internalWriteDefault = "internalWriteDefault"_sd;
}  // namespace 
...
ReadWriteConcernProvenanceSourceEnum ReadWriteConcernProvenanceSource_parse(const IDLParserErrorContext& ctxt, StringData value) {
    ...
    if (value == kReadWriteConcernProvenanceSource_internalWriteDefault) {
        return ReadWriteConcernProvenanceSourceEnum::internalWriteDefault;
    }
...
StringData ReadWriteConcernProvenanceSource_serializer(ReadWriteConcernProvenanceSourceEnum value) {
    ...
    if (value == ReadWriteConcernProvenanceSourceEnum::internalWriteDefault) {
        return kReadWriteConcernProvenanceSource_internalWriteDefault;
    }
    MONGO_UNREACHABLE;
    return StringData();
}
...
constexpr StringData WriteConcernIdl::kFsyncFieldName;
constexpr StringData WriteConcernIdl::kGetLastErrorFieldName;
constexpr StringData WriteConcernIdl::kJFieldName;
constexpr StringData WriteConcernIdl::kSourceFieldName;
constexpr StringData WriteConcernIdl::kWElectionIdFieldName;
constexpr StringData WriteConcernIdl::kWOpTimeFieldName;
constexpr StringData WriteConcernIdl::kWriteConcernWFieldName;
constexpr StringData WriteConcernIdl::kWtimeoutFieldName;


WriteConcernIdl::WriteConcernIdl()  {
    // Used for initialization only
}


WriteConcernIdl WriteConcernIdl::parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    auto object = mongo::idl::preparsedValue<WriteConcernIdl>();
    object.parseProtected(ctxt, bsonObject);
    return object;
}
void WriteConcernIdl::parseProtected(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    std::bitset<8> usedFields;
    const size_t kWriteConcernWBit = 0;
    const size_t kJBit = 1;
    const size_t kWtimeoutBit = 2;
    const size_t kFsyncBit = 3;
    const size_t kWElectionIdBit = 4;
    const size_t kWOpTimeBit = 5;
    const size_t kGetLastErrorBit = 6;
    const size_t kSourceBit = 7;

    for (const auto& element :bsonObject) {
        const auto fieldName = element.fieldNameStringData();


        if (fieldName == kWriteConcernWFieldName) {
            if (MONGO_likely(ctxt.checkAndAssertTypes(element, {String, NumberInt, NumberDecimal, NumberDouble, NumberLong}))) {
                if (MONGO_unlikely(usedFields[kWriteConcernWBit])) {
                    ctxt.throwDuplicateField(element);
                }

                usedFields.set(kWriteConcernWBit);

                _writeConcernW = WriteConcernW::deserializeWriteConcernW(element);
            }
        }
        else if (fieldName == kJFieldName) {
            if (MONGO_likely(ctxt.checkAndAssertTypes(element, {Bool, NumberLong, NumberInt, NumberDecimal, NumberDouble}))) {
                if (MONGO_unlikely(usedFields[kJBit])) {
                    ctxt.throwDuplicateField(element);
                }

                usedFields.set(kJBit);

                _j = element.trueValue();
            }
        }
        else if (fieldName == kWtimeoutFieldName) {
            if (MONGO_likely(ctxt.checkAndAssertTypes(element, {NumberLong, NumberInt, NumberDecimal, NumberDouble}))) {
                if (MONGO_unlikely(usedFields[kWtimeoutBit])) {
                    ctxt.throwDuplicateField(element);
                }

                usedFields.set(kWtimeoutBit);

                _wtimeout = element.safeNumberLong();
            }
        }
        else if (fieldName == kFsyncFieldName) {
            if (MONGO_likely(ctxt.checkAndAssertTypes(element, {Bool, NumberLong, NumberInt, NumberDecimal, NumberDouble}))) {
                if (MONGO_unlikely(usedFields[kFsyncBit])) {
                    ctxt.throwDuplicateField(element);
                }

                usedFields.set(kFsyncBit);

                _fsync = element.trueValue();
            }
        }
        else if (fieldName == kWElectionIdFieldName) {
            if (MONGO_unlikely(usedFields[kWElectionIdBit])) {
                ctxt.throwDuplicateField(element);
            }

            usedFields.set(kWElectionIdBit);

            // ignore field
        }
        else if (fieldName == kWOpTimeFieldName) {
            if (MONGO_unlikely(usedFields[kWOpTimeBit])) {
                ctxt.throwDuplicateField(element);
            }

            usedFields.set(kWOpTimeBit);

            // ignore field
        }
        else if (fieldName == kGetLastErrorFieldName) {
            if (MONGO_unlikely(usedFields[kGetLastErrorBit])) {
                ctxt.throwDuplicateField(element);
            }

            usedFields.set(kGetLastErrorBit);

            // ignore field
        }
        else if (fieldName == kSourceFieldName) {
            if (MONGO_likely(ctxt.checkAndAssertType(element, String))) {
    ...
    if (MONGO_unlikely(!usedFields.all())) {
        if (!usedFields[kWriteConcernWBit]) {
            _writeConcernW = mongo::WriteConcernW();
        }
        if (!usedFields[kWtimeoutBit]) {
            _wtimeout = 0;
        }
    }
...
void WriteConcernIdl::serialize(BSONObjBuilder* builder) const {
    {
        _writeConcernW.serializeWriteConcernW(kWriteConcernWFieldName, builder);
    }

    if (_j.is_initialized()) {
        builder->append(kJFieldName, _j.get());
    }

    builder->append(kWtimeoutFieldName, _wtimeout);

    if (_fsync.is_initialized()) {
        builder->append(kFsyncFieldName, _fsync.get());
    }
...
BSONObj WriteConcernIdl::toBSON() const {
    BSONObjBuilder builder;
    serialize(&builder);
    return builder.obj();
}
...
void Collation::parseProtected(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    ...
    if (MONGO_unlikely(!usedFields.all())) {
        ...
        if (!usedFields[kCaseLevelBit]) {
            _caseLevel = false;
        }
        if (!usedFields[kCaseFirstBit]) {
            _caseFirst = mongo::CollationCaseFirstEnum::kOff;
        }
        if (!usedFields[kStrengthBit]) {
            _strength = static_cast<int>(CollationStrength::kTertiary);
        }
        if (!usedFields[kNumericOrderingBit]) {
            _numericOrdering = false;
        }
        if (!usedFields[kAlternateBit]) {
            _alternate = mongo::CollationAlternateEnum::kNonIgnorable;
        }
        if (!usedFields[kMaxVariableBit]) {
            _maxVariable = mongo::CollationMaxVariableEnum::kPunct;
        }
        if (!usedFields[kNormalizationBit]) {
            _normalization = false;
        }
        if (!usedFields[kBackwardsBit]) {
            _backwards = mongo::OptionalBool();
        }
    }

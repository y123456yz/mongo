...
RemoteCursor::RemoteCursor() : _shardId(mongo::idl::preparsedValue<decltype(_shardId)>()), _hostAndPort(mongo::idl::preparsedValue<decltype(_hostAndPort)>()), _cursorResponse(mongo::idl::preparsedValue<decltype(_cursorResponse)>()), _hasShardId(false), _hasHostAndPort(false), _hasCursorResponse(false) {
...
RemoteCursor RemoteCursor::parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    auto object = mongo::idl::preparsedValue<RemoteCursor>();
...
AsyncResultsMergerParams::AsyncResultsMergerParams() : _nss(mongo::idl::preparsedValue<decltype(_nss)>()), _hasRemotes(false), _hasNss(false) {
    // Used for initialization only
}
...
AsyncResultsMergerParams AsyncResultsMergerParams::parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    auto object = mongo::idl::preparsedValue<AsyncResultsMergerParams>();
    object.parseProtected(ctxt, bsonObject);
    return object;
}
    ...
    for (const auto& element :bsonObject) {
        ...
        if (fieldName == kSessionIdFieldName) {
            if (MONGO_likely(ctxt.checkAndAssertType(element, Object))) {
                ...
                _operationSessionInfo.setSessionId(boost::optional<mongo::LogicalSessionFromClient>(mongo::LogicalSessionFromClient::parse(tempContext, localObject)));
            }
        }
            ...
            if (MONGO_likely(ctxt.checkAndAssertType(element, Bool))) {
                ...
                _operationSessionInfo.setAutocommit(boost::optional<bool>(element.boolean()));
            }
        }
            ...
            if (MONGO_likely(ctxt.checkAndAssertType(element, Bool))) {
                ...
                _operationSessionInfo.setStartTransaction(boost::optional<bool>(element.boolean()));
            }
        }
            ...
            if (MONGO_likely(ctxt.checkAndAssertType(element, Bool))) {
                ...
                _operationSessionInfo.setCoordinator(boost::optional<bool>(element.boolean()));
            }
        }
...
void AsyncResultsMergerParams::serialize(BSONObjBuilder* builder) const {
    ...
    if (_tailableMode.is_initialized()) {
        builder->append(kTailableModeFieldName, ::mongo::TailableMode_serializer(_tailableMode.get()));
    }

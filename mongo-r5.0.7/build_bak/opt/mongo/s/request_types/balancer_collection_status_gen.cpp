...
BalancerCollectionStatusResponse::BalancerCollectionStatusResponse() : _balancerCompliant(mongo::idl::preparsedValue<decltype(_balancerCompliant)>()), _hasBalancerCompliant(false) {
...
BalancerCollectionStatusResponse BalancerCollectionStatusResponse::parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    auto object = mongo::idl::preparsedValue<BalancerCollectionStatusResponse>();
...
void BalancerCollectionStatusResponse::parseProtected(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    ...
    const size_t kBalancerCompliantBit = 0;
    const size_t kFirstComplianceViolationBit = 1;
    ...
    for (const auto& element :bsonObject) {
        ...
        if (fieldName == kBalancerCompliantFieldName) {
            if (MONGO_likely(ctxt.checkAndAssertType(element, Bool))) {
                if (MONGO_unlikely(usedFields[kBalancerCompliantBit])) {
                    ctxt.throwDuplicateField(element);
                }

                usedFields.set(kBalancerCompliantBit);
            ...
            if (MONGO_likely(ctxt.checkAndAssertType(element, String))) {
                if (MONGO_unlikely(usedFields[kFirstComplianceViolationBit])) {
                    ctxt.throwDuplicateField(element);
                }

                usedFields.set(kFirstComplianceViolationBit);
    ...
    if (MONGO_unlikely(!usedFields.all())) {
        if (!usedFields[kBalancerCompliantBit]) {

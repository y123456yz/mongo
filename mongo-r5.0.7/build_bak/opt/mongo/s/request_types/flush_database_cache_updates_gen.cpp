...
constexpr StringData _flushDatabaseCacheUpdatesWithWriteConcern::kCommandParameterFieldName;
constexpr StringData _flushDatabaseCacheUpdatesWithWriteConcern::kDbNameFieldName;
constexpr StringData _flushDatabaseCacheUpdatesWithWriteConcern::kSyncFromConfigFieldName;
constexpr StringData _flushDatabaseCacheUpdatesWithWriteConcern::kCommandName;

const std::vector<StringData> _flushDatabaseCacheUpdatesWithWriteConcern::_knownBSONFields {
    _flushDatabaseCacheUpdatesWithWriteConcern::kSyncFromConfigFieldName,
    _flushDatabaseCacheUpdatesWithWriteConcern::kCommandName,
};
const std::vector<StringData> _flushDatabaseCacheUpdatesWithWriteConcern::_knownOP_MSGFields {
    _flushDatabaseCacheUpdatesWithWriteConcern::kDbNameFieldName,
    _flushDatabaseCacheUpdatesWithWriteConcern::kSyncFromConfigFieldName,
    _flushDatabaseCacheUpdatesWithWriteConcern::kCommandName,
};

_flushDatabaseCacheUpdatesWithWriteConcern::_flushDatabaseCacheUpdatesWithWriteConcern(const std::string commandParameter) : _commandParameter(std::move(commandParameter)), _hasDbName(false) {
    // Used for initialization only
}


_flushDatabaseCacheUpdatesWithWriteConcern _flushDatabaseCacheUpdatesWithWriteConcern::parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    auto localCmdType = mongo::idl::preparsedValue<std::string>();
    _flushDatabaseCacheUpdatesWithWriteConcern object(localCmdType);
    object.parseProtected(ctxt, bsonObject);
    return object;
}
void _flushDatabaseCacheUpdatesWithWriteConcern::parseProtected(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    std::bitset<2> usedFields;
    const size_t kSyncFromConfigBit = 0;
    const size_t kDbNameBit = 1;
    BSONElement commandElement;
...
_flushDatabaseCacheUpdatesWithWriteConcern _flushDatabaseCacheUpdatesWithWriteConcern::parse(const IDLParserErrorContext& ctxt, const OpMsgRequest& request) {
    auto localCmdType = mongo::idl::preparsedValue<std::string>();
    _flushDatabaseCacheUpdatesWithWriteConcern object(localCmdType);
    object.parseProtected(ctxt, request);
    return object;
}
void _flushDatabaseCacheUpdatesWithWriteConcern::parseProtected(const IDLParserErrorContext& ctxt, const OpMsgRequest& request) {
    std::bitset<2> usedFields;
    const size_t kSyncFromConfigBit = 0;
    const size_t kDbNameBit = 1;
    BSONElement commandElement;
...
void _flushDatabaseCacheUpdatesWithWriteConcern::serialize(const BSONObj& commandPassthroughFields, BSONObjBuilder* builder) const {
    invariant(_hasDbName);
...
OpMsgRequest _flushDatabaseCacheUpdatesWithWriteConcern::serialize(const BSONObj& commandPassthroughFields) const {
    BSONObjBuilder localBuilder;
    {
        BSONObjBuilder* builder = &localBuilder;
        invariant(_hasDbName);
...
BSONObj _flushDatabaseCacheUpdatesWithWriteConcern::toBSON(const BSONObj& commandPassthroughFields) const {
    BSONObjBuilder builder;
    serialize(commandPassthroughFields, &builder);
    return builder.obj();
}

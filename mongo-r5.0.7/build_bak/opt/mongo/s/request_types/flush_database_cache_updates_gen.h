 ...
 */
class _flushDatabaseCacheUpdatesWithWriteConcern {
public:
    using Reply = void;
    static constexpr auto kCommandParameterFieldName = "_flushDatabaseCacheUpdatesWithWriteConcern"_sd;
    static constexpr auto kDbNameFieldName = "$db"_sd;
    static constexpr auto kSyncFromConfigFieldName = "syncFromConfig"_sd;
    static constexpr auto kCommandName = "_flushDatabaseCacheUpdatesWithWriteConcern"_sd;

    explicit _flushDatabaseCacheUpdatesWithWriteConcern(const std::string commandParameter);

    static _flushDatabaseCacheUpdatesWithWriteConcern parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject);
    static _flushDatabaseCacheUpdatesWithWriteConcern parse(const IDLParserErrorContext& ctxt, const OpMsgRequest& request);
    void serialize(const BSONObj& commandPassthroughFields, BSONObjBuilder* builder) const;
    OpMsgRequest serialize(const BSONObj& commandPassthroughFields) const;
    BSONObj toBSON(const BSONObj& commandPassthroughFields) const;

...
public:
    ...
    static constexpr auto kCommandIsFromRouterFieldName = "commandIsFromRouter"_sd;
    static constexpr auto kToFieldName = "to"_sd;
    ...
    /**
     * Indicates whether the command has been sent by a router
     */
    bool getCommandIsFromRouter() const { return _commandIsFromRouter; }
    void setCommandIsFromRouter(bool value) & {  _commandIsFromRouter = std::move(value);  }


    friend bool operator==(const ShardMovePrimary& left, const ShardMovePrimary& right) {
        return left.__shardsvrMovePrimary == right.__shardsvrMovePrimary && left._to == right._to && left._commandIsFromRouter == right._commandIsFromRouter;
    }
    ...
    friend bool operator<(const ShardMovePrimary& left, const ShardMovePrimary& right) {
        return left.__shardsvrMovePrimary < right.__shardsvrMovePrimary || (!(right.__shardsvrMovePrimary < left.__shardsvrMovePrimary) && (left._to < right._to || (!(right._to < left._to) && (left._commandIsFromRouter < right._commandIsFromRouter))));
    }
    friend bool operator>(const ShardMovePrimary& left, const ShardMovePrimary& right) {
        return right < left;
    }
...
private:
    ...
    std::string _to;
    bool _commandIsFromRouter{false};

...
CommitTypeStats CommitTypeStats::parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    auto object = mongo::idl::preparsedValue<CommitTypeStats>();
...
CommitTypes::CommitTypes() : _noShards(mongo::idl::preparsedValue<decltype(_noShards)>()), _singleShard(mongo::idl::preparsedValue<decltype(_singleShard)>()), _singleWriteShard(mongo::idl::preparsedValue<decltype(_singleWriteShard)>()), _readOnly(mongo::idl::preparsedValue<decltype(_readOnly)>()), _twoPhaseCommit(mongo::idl::preparsedValue<decltype(_twoPhaseCommit)>()), _recoverWithToken(mongo::idl::preparsedValue<decltype(_recoverWithToken)>()), _hasNoShards(false), _hasSingleShard(false), _hasSingleWriteShard(false), _hasReadOnly(false), _hasTwoPhaseCommit(false), _hasRecoverWithToken(false) {
    // Used for initialization only
}
...
CommitTypes CommitTypes::parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    auto object = mongo::idl::preparsedValue<CommitTypes>();
    object.parseProtected(ctxt, bsonObject);
    return object;
}
...
RouterTransactionsStats::RouterTransactionsStats() : _abortCause(mongo::idl::preparsedValue<decltype(_abortCause)>()), _commitTypes(mongo::idl::preparsedValue<decltype(_commitTypes)>()), _hasAbortCause(false), _hasCommitTypes(false) {
    // Used for initialization only
}
...
RouterTransactionsStats RouterTransactionsStats::parse(const IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {
    auto object = mongo::idl::preparsedValue<RouterTransactionsStats>();
    object.parseProtected(ctxt, bsonObject);
    return object;
}
